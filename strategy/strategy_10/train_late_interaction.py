...
def evaluate_retrieval(model, loader, device):
    ...
    if model.use_colbert:
        ...
        for graphs, text_emb in loader:
            graphs, text_emb = graphs.to(device), text_emb.to(device)
            all_g.append(F.normalize(model.forward_graph(graphs, return_tokens=False), p=2, dim=-1))
            all_t.append(F.normalize(model.forward_text(text_emb, return_tokens=False), p=2, dim=-1))
...
if __name__ == "__main__":
    ...
    if USE_COLBERT_EMBEDDINGS:
        ...
        # When using ColBERT embeddings, text_dim should be num_tokens * hidden_dim
        text_input_dim = NUM_TOKENS * HIDDEN_DIM_EMB
        print(f"  → Loaded token embeddings: {NUM_TOKENS} tokens × {HIDDEN_DIM_EMB} dim")
    ...
    train_ds = PreprocessedGraphDataset("/kaggle/input/molecular-data/train_graphs.pkl", train_emb)
